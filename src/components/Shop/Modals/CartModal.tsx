...
export const deleteCart = async ({ id, removeFromCart }: { id: number; removeFromCart: (id: number) => void }) => {
  try {
...
const CartPreview = ({ id, product, variant, quantity }: CartItemType & { id: number }) => {
  ...
  return (
    <div className="px-10 flex gap-x-3 mb-4 group">
      ...
      <div className="flex flex-col flex-1">
        <h1 className="text-[17px] font-medium tracking-wider">${price.toFixed(2)}</h1>
      ...
      <main className="content">
        {carts.length > 0 &&
          carts.map((cart, index) => {
            return <CartPreview key={`${cart.id}--${index}--${new Date().getMilliseconds()}`} {...cart} />
          })}

        {carts.length < 1 && <div className="text-center my-4">Your cart is empty</div>}
      </main>
