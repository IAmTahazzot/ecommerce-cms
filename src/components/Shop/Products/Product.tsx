...
export const buyProduct = async ({
  ...
  if (existingCartItem) {
    // duplicate product
    toast.info('Product already in cart. You may update the quantity in the cart.', {
...
export const Product = ({ product }: { product: ProductType }) => {
  ...
  const reviewAvg =
    product.Review &&
    product.Review.reduce((acc, review) => acc + Number(review.reviewLevel), 0) / product.Review.length

  const reviews = product.Review && (
    <div className="flex items-center mt-2">
      {Array.from({ length: 5 }).map((_, index) => {
        if (!reviewAvg) return <IoStarSharp key={index} className="text-neutral-300" />
    ...
    <div>
      <div className="flex flex-col md:flex-row justify-between gap-5">
        <div className="flex-1 flex flex-col-reverse lg:flex-row gap-4">
          <div className="flex flex-row lg:flex-col gap-4 lg:w-24">
            {product.images.map((image, index) => {
              return (
                <div
                  ...
                  className={cn(
                    'w-full aspect-square relative rounded-md overflow-hidden cursor-pointer',
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        selectedVariant?.size === size ? 'border-2 border-black' : 'border-neutral-300'
                      )}
                    >
                      {size}
                    </button>
                ...
                {variants.colors.map((color, index) => {
                  return (
                    <button
                      ...
                      className={cn(
                        'rounded-full py-3 px-8 border border-spacing-2 hover:border-black',
                        selectedVariant?.color === color ? 'border-2 border-black' : 'border-neutral-300'
                      )}
                    >
                      {color}
                    </button>
                ...
                {variants.materials.map((material, index) => {
                  return (
                    <button
                      ...
                      className={cn(
                        'rounded-full py-3 px-8 border border-spacing-2 hover:border-black',
                        selectedVariant?.material === material ? 'border-2 border-black' : 'border-neutral-300'
                      )}
                    >
                      {material}
                    </button>
      ...
      <div>
        <div className="border-b border-neutral-200 pt-6 flex items-end gap-x-4">
          <Button
            variant="ghost"
            className={cn('rounded-b-none py-6', view === 'description' && 'border-b-2 border-black')}
          ...
          <Button
            variant="ghost"
            className={cn('rounded-b-none py-6', view === 'reviews' && 'border-b-2 border-black')}
        ...
        <div className="mt-10">
          ...
          {view === 'reviews' && (
            <div>
              <div className="my-10">
                ...
                <div className="flex items-center gap-x-4 mb-6">
                  ...
                  <div className="flex items-center">
                    {Array.from({ length: 5 }).map((_, index) => {
                      if (!reviewAvg) {
                        return <IoStarSharp size={20} key={index} className="text-neutral-300" />
                      }
                  ...
                  <span className="text-lg">({product.Review && product.Review.length})</span>
                </div>
                ...
                <div className="flex items-start gap-x-4 flex-col-reverse">
                  {Array.from({ length: 5 }).map((_, index) => {
                    const count =
                      product.Review &&
                      product.Review.filter((review) => Number(review.reviewLevel) === index + 1).length
                        ...
                        <div className="grid grid-cols-[50px_1fr_1fr] gap-4 items-center">
                          ...
                          <div className="relative bg-neutral-200 rounded-full overflow-hidden h-3 w-[300px]">
                            <div
                              ...
                              style={{
                                width: `${count && product.Review && (count / product.Review.length) * 100}%`,
                              }}
                            ></div>
                          </div>
              ...
              product.Review.find((review) => review.userId === user.id) ? (
                <p className="text-sm my-3">You have already reviewed this product</p>
              ) : (
                <div>
                  <div>
                    <h2 className="text-3xl font-medium mb-4">Write a review</h2>
          ...
          {view === 'reviews' && product.Review && product.Review.length > 0 && reviewAvg && (
            <div>
              <div className="mt-10">
