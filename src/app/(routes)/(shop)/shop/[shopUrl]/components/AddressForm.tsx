...
const AddressFormSchema = z.object({
  ...
  phoneNumber: z.string().min(9, { message: 'Phone number must be at least 9 characters long' }),
  state: z.string().optional(),
  zip: z.string().min(4, { message: 'Zip code must be at least 4 characters long' }),
})

export const AddressForm = ({ address, isModal = false }: { address: Address | undefined; isModal?: boolean }) => {
          ...
          <form onSubmit={form.handleSubmit(submit)}>
            <div className="flex gap-x-6">
              <FormItem className="flex-1">
                ...
                <FormMessage className="h-10">{form.formState.errors.country?.message}</FormMessage>
              </FormItem>
              <FormItem className="flex-1">
            ...
            <FormItem className="mt-6">
              ...
              <Input id="phoneNumber" placeholder="Enter your phone number..." {...form.register('phoneNumber')} />
            ...
            <FormItem className="mt-6">
              ...
              />
              <FormMessage>{form.formState.errors.addressOne?.message}</FormMessage>
            </FormItem>
            <FormItem className="mt-6">
              <FormLabel htmlFor="address" className="block">
                Address Line 2
              ...
              />
              <FormMessage>{form.formState.errors.addressTwo?.message}</FormMessage>
            </FormItem>

            {isModal ? (
              <div className="flex justify-end">
                <Button variant={'default'} className="h-12 mt-7" type="submit">
                  {form.formState.isSubmitting ? 'Saving...' : 'Save & Checkout'}
                </Button>
              </div>
            ) : (
              <Button variant={'default'} className="w-24 h-12 mt-7" type="submit">
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button id="country" variant="outline" role="combobox" aria-expanded={open} className="w-full justify-between">
          {value
            ? Object.entries(data).find(([country]) => country.toLowerCase() === value)?.[0] ?? value
            : 'Select country...'}
            ...
            {Object.entries(data).map(([country]) => (
              <CommandItem
                ...
                onSelect={(currentValue) => {
                  setValue(currentValue.toLowerCase() === value ? value : currentValue)
...
const SelectCity = ({ form, country }: { form: any; country: keyof typeof data | '' }) => {
  const [open, setOpen] = React.useState(false)
  ...
  useEffect(() => {
    if (country !== '') {
      ...
      const country = Object.entries(data).find(([c]) => c.toLowerCase() === form.getValues('country'))
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button id="city" variant="outline" role="combobox" aria-expanded={open} className="w-full justify-between">
          {value ? cities.find((city) => city.toLowerCase() === value) ?? value : 'Select city...'}
          <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
        </Button>
      </PopoverTrigger>
      <PopoverContent className="w-full p-0">
              ...
              cities.map((city) => (
                <CommandItem
                  ...
                  onSelect={(currentValue) => {
                    ...
                    const originalCityName = cities.find((city) => city.toLowerCase() === currentValue)
